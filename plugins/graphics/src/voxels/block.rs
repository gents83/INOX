...
impl Block {
    ...
    const FACE_ORDER: [usize; Block::FACE_COUNT] = [
        0, 3, 1, 1, 3, 2
    ];
    ...
    fn get_config(&self) -> &BlockConfig { unsafe{ &CONFIG[self.0 as usize] } }

    pub fn is_transparent(&self) -> bool { self.get_config().is_transparent }

    pub fn is_active(&self) -> bool { self.get_config().is_active }

    pub fn is_invisible(&self) -> bool { !self.is_active() }

    
    pub fn generate_mesh(&self, vertices: &mut Vec<VertexData>, x: f32, y: f32, z: f32, sides: u8, edges: u32, corners: u8) {
