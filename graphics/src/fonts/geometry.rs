...
pub struct Geometry<'a> {
    ...
    offset: Vector2f,
    scale: Vector2f,
    current_point: Vector2f,
    last_movement: Option<Vector2f>,
    lines: Vec<Line>,
}

impl<'a> Geometry<'a> {
    pub fn new(offset: Vector2f, scale: Vector2f, width: usize, height: usize, data: &'a mut Vec<f32>) -> Self {
        Self {
            rasterizer: Raster::new(width, height, data),
            offset,
            scale,
            current_point: Vector2f::default(),
            last_movement: None,
            lines: Vec::new(),
        }
    }
...
impl<'a> OutlineBuilder for Geometry<'a> {
    fn move_to(&mut self, x0: f32, y0: f32) {
        let next_point = Vector2f::new((x0+ self.offset.x) * self.scale.x, (y0 + self.offset.y) * self.scale.y);
    ...
    fn quad_to(&mut self, x0: f32, y0: f32, x1: f32, y1: f32) {
        let control_point = Vector2f::new((x0+ self.offset.x) * self.scale.x, (y0 + self.offset.y) * self.scale.y);
        let next_point = Vector2f::new((x1 + self.offset.x) * self.scale.x, (y1 + self.offset.y) * self.scale.y);
    ...
    fn curve_to(&mut self, x0: f32, y0: f32, x1: f32, y1: f32, x2: f32, y2: f32) {
        let first_control = Vector2f::new((x0+ self.offset.x) * self.scale.x + self.offset.x, y0 * self.scale.y + self.offset.y);
        let second_control = Vector2f::new((x1 + self.offset.x) * self.scale.x, (y1 + self.offset.y) * self.scale.y);
        let next_point = Vector2f::new((x2 + self.offset.x) * self.scale.x, (y2 + self.offset.y) * self.scale.y);

        self.draw_cubic(self.current_point, first_control, second_control, next_point);
        self.current_point = next_point;
    }
...
impl<'a> Geometry<'a> {
    fn draw_line(&mut self, start: Vector2f, end: Vector2f) {
        self.lines.push(Line{start,end});
        self.rasterizer.draw_line(start, end);
    }

    fn draw_quad(&mut self, p0: Vector2f, p1: Vector2f, p2: Vector2f) {
    ...
    fn draw_cubic(&mut self, p0: Vector2f, p1: Vector2f, p2: Vector2f, p3: Vector2f) {
        self.tesselate_cubic(p0, p1, p2, p3, 0);
    }

    fn tesselate_cubic(&mut self, p0: Vector2f, p1: Vector2f, p2: Vector2f, p3: Vector2f, n: u8) {
