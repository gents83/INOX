...
fn main() {
  ...
  let commands_v12:HashSet<_> = commands_v12.difference(&commands_extensions).collect();
  let commands_v11:HashSet<_> = commands_v11.difference(&commands_extensions).collect();
  let commands_v10:HashSet<_> = commands_v10.difference(&commands_extensions).collect();
  
  writeln!(f, "").unwrap();

  writeln!(f, "pub struct LibLoader {{").unwrap();
  for command in commands_v10.clone() {
      writeln!(f, "    pub {}: PFN_{},", command, command).unwrap();
  }
  for command in commands_v11.clone() {
      writeln!(f, "    pub {}: PFN_{},", command, command).unwrap();
  }
  for command in commands_v12.clone() {
      writeln!(f, "    pub {}: PFN_{},", command, command).unwrap();
  }
  writeln!(f, "}}").unwrap();
