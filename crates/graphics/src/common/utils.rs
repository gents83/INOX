...
use crate::{GPUBHVNode, INVALID_INDEX};
...
pub fn create_linearized_bhv(bhv: &BHVTree) -> Vec<GPUBHVNode> {
    inox_profiler::scoped_profile!("create_linearized_bhv");

    let mut linearized_bhv = Vec::<GPUBHVNode>::new();
    ...
    while current_index < nodes_count {
        ...
        let mut linearized_node = GPUBHVNode {
