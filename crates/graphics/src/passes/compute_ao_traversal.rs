...
pub const COMPUTE_AO_TRAVERSAL_PIPELINE: &str = "pipelines/ComputeAOTraversal.compute_pipeline";
pub const COMPUTE_AO_TRAVERSAL_NAME: &str = "ComputeAOTraversalPass";

pub struct ComputeAOTraversalPass {
...
unsafe impl Send for ComputeAOTraversalPass {}
unsafe impl Sync for ComputeAOTraversalPass {}

impl Pass for ComputeAOTraversalPass {
    fn name(&self) -> &str {
        COMPUTE_AO_TRAVERSAL_NAME
    }
        ...
        Self: Sized,
    {
        let data = ComputePassData {
            name: COMPUTE_AO_TRAVERSAL_NAME.to_string(),
        ...
        Self {
            ...
            meshlets,
            bvh,
            binding_data: BindingData::new(render_context, COMPUTE_AO_TRAVERSAL_NAME),
        }
    }
    ...
    fn init(&mut self, render_context: &RenderContext) {
        ...
        // Group 3: Ray data (using AO rays instead of shadow rays)
        self.binding_data
            .add_buffer(
                &mut *self.ao_rays.write().unwrap(),
                Some("shadow_rays"), // Keep name shadow_rays to match shader
                BindingInfo {
                    group_index: 3,
                    binding_index: 0,
                    stage: ShaderStage::Compute,
            ...
            .add_buffer(
                ...
                Some("shadow_intersections"), // Keep name shadow_intersections to match shader
                BindingInfo {
                    group_index: 3,
                    binding_index: 1,
                    stage: ShaderStage::Compute,
    ...
    fn update(
        ...
        let ray_count = self.ao_rays.read().unwrap().len() as u32;
        if ray_count == 0 {
            return;
        }
...
impl ComputeAOTraversalPass {
    pub fn ao_intersections(&self) -> &GPUVector<IntersectionPackedData> {
        &self.ao_intersections
    }
}
