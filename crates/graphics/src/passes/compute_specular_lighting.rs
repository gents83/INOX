...
pub const COMPUTE_SPECULAR_LIGHTING_PIPELINE: &str =
    "pipelines/ComputeSpecularLighting.compute_pipeline";
pub const COMPUTE_SPECULAR_LIGHTING_NAME: &str = "ComputeSpecularLightingPass";

pub struct ComputeSpecularLightingPass {
...
unsafe impl Send for ComputeSpecularLightingPass {}
unsafe impl Sync for ComputeSpecularLightingPass {}

impl Pass for ComputeSpecularLightingPass {
    fn name(&self) -> &str {
        COMPUTE_SPECULAR_LIGHTING_NAME
    }
    ...
    fn create(context: &ContextRc, render_context: &RenderContextRc) -> Self {
        let data = ComputePassData {
            name: COMPUTE_SPECULAR_LIGHTING_NAME.to_string(),
            pipelines: vec![PathBuf::from(COMPUTE_SPECULAR_LIGHTING_PIPELINE)],
        };
        ...
        Self {
            ...
            binding_data: BindingData::new(render_context, COMPUTE_SPECULAR_LIGHTING_NAME),
            dimensions: (0, 0),
        }
    }

    fn init(&mut self, render_context: &RenderContext) {
        inox_profiler::scoped_profile!("specular_lighting_pass::init");

        if self.meshlets.read().unwrap().is_empty() {
            return;
        }

        // Group 0: geometry (same as diffuse)
        self.binding_data
            .add_buffer(
        ...
        // Group 1: materials, textures, lights, data buffers
        self.binding_data
            .add_buffer(
    ...
    fn update(
        ...
        inox_profiler::scoped_profile!("specular_lighting_pass::update");
...
impl ComputeSpecularLightingPass {
