...
impl Context {
    ...
    pub fn global_timer(&'_ self) -> RwLockReadGuard<'_, Timer> {
        self.global_timer.read().unwrap()
    }
    pub fn global_timer_mut(&'_ self) -> RwLockWriteGuard<'_, Timer> {
        self.global_timer.write().unwrap()
    }
    pub(crate) fn scheduler_mut(&'_ self) -> RwLockWriteGuard<'_, Scheduler> {
