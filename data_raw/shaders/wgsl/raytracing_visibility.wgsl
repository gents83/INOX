...
fn compute_ray(image_pixel: vec2<u32>, image_size: vec2<u32>) -> Ray {
    ...
    let ray = Ray(origin, 0., direction, 1.);
    return ray;
}
...
fn main(
    ...
    var nearest = Intersection(ray.origin, f32(0xFFFFFFFFu), ray.origin, 0u);

    let mesh_count = 8u;//arrayLength(&meshes.data);    
    for (var mesh_id = 0u; mesh_id < mesh_count; mesh_id++) {
        ...
        for (var meshlet_index = 0u; meshlet_index < (*mesh).meshlets_count; meshlet_index++) {
            ...
            for (var primitive_id = 0u; primitive_id < triangle_count; primitive_id++) {
                ...
                if (hit) {
                    nearest.visibility_id = 0xFFFFFFFFu;//((meshlet_id + 1u) << 8u) + primitive_id;
                }
                //Intersection found
    ...
    textureStore(render_target, vec2<i32>(pixel), unpack4x8unorm(nearest.visibility_id));
}