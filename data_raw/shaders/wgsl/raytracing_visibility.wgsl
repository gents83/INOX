...
fn fs_main(v_in: VertexOutput) -> @location(0) vec4<f32> {
    let pixel_coords = vec2<i32>(i32(v_in.uv.x * constant_data.screen_width), 
                                 i32(v_in.uv.y * constant_data.screen_height));

    let visibility_id = 0u;
    let texture_color = unpack4x8unorm(visibility_id);     
