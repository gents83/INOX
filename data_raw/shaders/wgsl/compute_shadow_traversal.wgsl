...
// Optimized shadow ray traversal - any-hit mode
fn traverse_bvh_shadow(ray_origin: vec3<f32>, ray_direction: vec3<f32>, t_max: f32, tlas_starting_index: u32) -> bool {
    ...
    while(stack_ptr > 0 && loop_count < 2048u) {
        ...
        if(intersection < t_max) {
            if(node.reference < 0) {
                ...
                // Simple BLAS traversal for shadow - any hit
                if(traverse_blas_any_hit(
                    ...
                    return true; // Early exit - we hit something
                }
            }
        }
    }
...
// Simplified BLAS traversal for shadow rays - any hit
...
fn main(
    ...
    // Perform any-hit BVH traversal for shadow ray
    let is_occluded = traverse_bvh_shadow(ray.origin, ray.direction, ray.t_max, constant_data.tlas_starting_index);
    
    // Write result - for shadow rays we only need binary visibility
    if (is_occluded) {
