...
impl TextBox {
    pub fn editable(&mut self, is_editable: bool) -> &mut Self {
        if self.is_editable != is_editable {
            ...
            if let Some(text_panel) = self.node().get_child_mut::<Panel>(uid) {
    ...
    pub fn set_text(&mut self, text: &str) -> &mut Self {
        let uid = self.editable_text;
        if let Some(editable_text) = self.node().get_child_mut::<Text>(uid) {
    ...
    fn update_character_from_indicator(&mut self) {
        ...
        if let Some(text) = self.node().get_child_mut::<Text>(text_widget_id) {
    ...
    fn update_indicator_position(&mut self) {
        ...
        if let Some(text) = self.node().get_child_mut::<Text>(text_widget_id) {
            ...
            if let Some(indicator) = self.node().get_child_mut::<Indicator>(indicator_id) {
...
impl InternalWidget for TextBox {
    ...
    fn widget_process_message(&mut self, msg: &dyn Message) {
        if msg.type_id() == TypeId::of::<MouseEvent>() {
            ...
            let mut current_char = self.current_char;
            if let Some(text) = self.node().get_child_mut::<Text>(text_id) {
